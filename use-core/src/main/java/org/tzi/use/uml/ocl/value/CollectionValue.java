...
public abstract class CollectionValue extends Value implements Iterable<Value> {
    ...
    /**
     * The probability that an element equals <code>value</code>, for the uncertain
     * membership operations below. Whichever side carries uncertainty decides;
     * a subjective operand is projected onto its probability, which is the
     * identity for every UBoolean-valued case.
     */
    private static UBooleanValue equality(Value element, Value value) {
        if (element instanceof UncertainValue u) return u.uEquals(value).toUBoolean();
        if (value instanceof UncertainValue u) return u.uEquals(element).toUBoolean();
        return UBooleanValue.probability(element.equals(value) ? 1 : 0);
    }

    /** Uncertain membership: the best match anywhere in the collection. */
    public UBooleanValue uIncludes(Value value) {
        UBooleanValue result = UBooleanValue.FALSE;
        for (Value element : this) {
            UBooleanValue candidate = equality(element, value);
    ...
    /**
     * The short circuit on size is wrong for an argument holding duplicates -- a
     * two-element receiver can include a three-element bag of two distinct values.
     * It is byte for byte the historical implementation and is kept deliberately.
     */
    public UBooleanValue uIncludesAll(CollectionValue other) {
        ...
        for (Value v : other) {
            result = result.and(uIncludes(v));
            if (result.probability() == 0) break;
        }
        return result;
    }

    /** Uncertain non-membership: every element has to differ. */
    public UBooleanValue uExcludes(Value value) {
        ...
        for (Value element : this) {
            ...
            result = result.and(candidate);
            if (result.probability() == 0) break;
        }
        return result;
    }

    public UBooleanValue uExcludesAll(CollectionValue other) {
        UBooleanValue result = UBooleanValue.TRUE;
        for (Value v : other) {
            result = result.and(uExcludes(v));
            if (result.probability() == 0) break;
        }
        return result;
    }

    /** How many elements equal <code>value</code> with at least this confidence. */
    public int uCountC(Value value, double confidence) {
        ...
        int n = 0;
        for (Value element : this) {
            if (equality(element, value).probability() >= confidence) n++;
        }
        return n;
    }

    /** {@link #uCountC} at the default confidence of one half. */
    public int uCount(Value value) {
        return uCountC(value, .5);
    }
