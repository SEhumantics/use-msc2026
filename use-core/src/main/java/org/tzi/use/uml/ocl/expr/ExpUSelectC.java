...
public final class ExpUSelectC extends ExpQuery {
    ...
    public String name(){return "uSelectC";} public Value eval(EvalContext c){c.enter(this);Value t=threshold.eval(c);if(t.isUndefined()){c.exit(this,UndefinedValue.instance);return UndefinedValue.instance;}double x=t instanceof IntegerValue i?i.value():((RealValue)t).value();if(x<0||x>1)throw new IllegalArgumentException("uSelectC confidence must be between 0 and 1");Value v=evalUSelect(c,x);c.exit(this,v);return v;} public void processWithVisitor(ExpressionVisitor v){v.visitUSelectC(this);}
}
