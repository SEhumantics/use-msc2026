...
public final class StandardOperationsUncertainty {
    ...
    private static void uRealMinMax(Multimap<String,OpGeneric>m,String n,java.util.function.BiFunction<URealValue,URealValue,URealValue> f){m.put(n,new Base(n,TypeFactory.mkUReal(),a->f.apply(ur(a[0]),ur(a[1])),false){public Type matches(Type[] x){return x.length==2&&x[0].isKindOfUReal(Type.VoidHandling.EXCLUDE_VOID)&&x[1].isKindOfUReal(Type.VoidHandling.EXCLUDE_VOID)&&(x[0].isTypeOfUReal()||x[1].isTypeOfUReal())?r:null;}});}
