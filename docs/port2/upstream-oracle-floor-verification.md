...
> |---|---|---|
> | **F-01** | seven of `exec:exec`'s **22** user-property parameters pinned in both poms; eight `exec.*` user properties handed back to the checker so that *setting one fails the build*; a Jupiter test (`UpstreamOracleGateWiringTest`) that no `exec` property can reach; a receipt verified after Maven exits. `-Dexec.args=-version` is now **`BUILD FAILURE`, exit 1, 16 `[floor]` lines**. §3.9's documented residual hole (both exec blocks deleted) is **also** closed. **CORRECTED 2026-08-18 (round 11 G-03):** this row said "all eight `exec:exec` user properties"; the descriptor declares **22** (measured — `upstream-oracle-profile.md` §5.2.1), seven pinned, eight detected, fourteen neither and deliberately so ([`gate-threat-model.md`](gate-threat-model.md) §3). **And round 11 refuted "setting one fails the build" on four routes; those are closed in round 12** — `upstream-oracle-profile.md` §5.2.6. | `upstream-oracle-profile.md` §5.2, §5.2.6, §7.4.2–7.4.4 |
