...
* **Constructor**: `ExpConstUReal(Expression eValue, Expression eUncertainty)` (`:13-17`) — **does not declare `throws ExpInvalidException` and performs no type checking at all.** It is the only one of the five with no guard. A `ExpConstUReal(stringExpr, objectExpr)` constructs happily and blows up at `eval`. Flag this in the port; ~~the fix is out of scope for a faithful copy~~ **SUPERSEDED 2026-08-17 (B7, defect F-06): the decision is FIX the historical defects, documenting each — bug-for-bug faithfulness was the recorded recommendation and was NOT taken.** So the missing guard IS in scope: add it, and record the fix and its observable consequence per `b7-fix-plan.md`. The struck sentence is kept visible as history because it was a live instruction until this line was corrected; do not act on it. It must still be listed as a known defect of the fork either way.
