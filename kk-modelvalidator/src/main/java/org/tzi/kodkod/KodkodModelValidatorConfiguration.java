...
public class KodkodModelValidatorConfiguration {
		...
		if (!read) {
			if (iniSaveFile.exists() && iniSaveFile.canRead()) {
				try {
					...
					// see ConfigurablePlugin.readConfiguration for why this is needed (config2 no
					// longer splits comma-delimited list values by default like config1 did).
					config.setListDelimiterHandler(new LegacyListDelimiterHandler(','));
					try (FileReader iniReader = new FileReader(iniSaveFile)) {
						config.read(iniReader);
	...
	public void saveFile() throws ConfigurationException {
		...
		try (FileWriter iniWriter = new FileWriter(iniSaveFile)) {
			iniFile.write(iniWriter);
		} catch (IOException ex) {
			throw new ConfigurationException(ex);
		}
		read = true;
