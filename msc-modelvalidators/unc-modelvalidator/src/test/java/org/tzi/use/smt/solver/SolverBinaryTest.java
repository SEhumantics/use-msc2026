...
public class SolverBinaryTest {
  ...
  public void resolvesTheVendoredBinaryFromAnyWorkingDirectory() {
    ...
    assertTrue(
        "must resolve the vendored copy, not one from PATH: " + binary.path(),
  ...
  public void reportedVersionMatchesThePinFile() throws Exception {
    ...
    assertEquals(
        pin.getProperty("solver.expectedVersion").strip(),
  ...
  /**
   * Regression guard for the blocking-read bug fixed 2026-08-24 (master plan Appendix B): {@code
   * probeVersion} used to call {@code readAllBytes()} on the child's stdout before {@code
   * waitFor(timeout)}, which blocks until the child exits and makes the timeout unreachable. A hung
   * solver would hang solver resolution forever instead of failing after the configured timeout.
   *
   * <p>Points {@code resolveFrom} at a script that ignores its arguments and sleeps, so {@code
   * binary --version} never returns. Uses the package-visible timeout overload with a short
   * duration so the test is fast and deterministic rather than waiting out the real 30-second
   * production timeout.
   */
  @Test(timeout = 15_000)
