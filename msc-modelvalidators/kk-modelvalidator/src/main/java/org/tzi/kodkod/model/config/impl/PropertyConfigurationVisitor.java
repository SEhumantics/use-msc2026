...
public class PropertyConfigurationVisitor extends SimpleVisitor {
	...
	public PropertyConfigurationVisitor(String file, PrintWriter warningsOut) throws ConfigurationException {
		this(loadPropertiesConfiguration(file), warningsOut);
	}

	private static PropertiesConfiguration loadPropertiesConfiguration(String file) throws ConfigurationException {
		PropertiesConfiguration pc = new PropertiesConfiguration();
		// see ConfigurablePlugin.readConfiguration for why this is needed (config2 no longer splits
		// comma-delimited "Set{a,b,c}" list values by default like config1 did); this visitor's own
		// readSingleElements()/readComplexElements() depend on that splitting.
		pc.setListDelimiterHandler(new LegacyListDelimiterHandler(','));
