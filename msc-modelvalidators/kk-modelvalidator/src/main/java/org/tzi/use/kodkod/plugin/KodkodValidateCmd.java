...
public class KodkodValidateCmd extends ConfigurablePlugin implements IPluginShellCmdDelegate {
	...
	private void validate(KodkodModelValidator modelValidator) {
		...
		reportOutcomeDirectly(modelValidator);
	}

	/**
	 * Prints the validation outcome directly to the invoking shell, bypassing this plugin's log4j
	 * loggers. In a multi-plugin distribution, log4j output can silently fail to reach the console
	 * (a pre-existing use-gui classloading issue, not something this plugin can fix from here -- see
	 * docs/kk-modelvalidator-port.md), so this is the reliable channel for whether validation found
	 * anything, independent of whichever log4j Hierarchy instance this invocation happened to bind to.
	 */
	private void reportOutcomeDirectly(KodkodModelValidator modelValidator) {
		if (useShell == null || modelValidator.solution() == null) {
			return;
		}
		useShell.getOut().println("[modelvalidator] outcome: " + modelValidator.solution().outcome());
	}
