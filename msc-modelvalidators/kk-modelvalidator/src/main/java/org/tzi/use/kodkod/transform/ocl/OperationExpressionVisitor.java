...
public class OperationExpressionVisitor extends DefaultExpressionVisitor {
	...
	public void visitInstanceOp(ExpInstanceOp exp) {
		...
		try {
			...
			// operation.cls() now returns MClassifier (widened upstream to also cover interfaces/
			// datatypes); getOverriddenOperations() only ever walks concrete-class inheritance, which
			// is what this plugin's supported fragment operates on.
			Map<MClass, MOperation> overiddenOperations = getOverriddenOperations((MClass) operation.cls(), operation.name());
